        ...
        <div class="d-flex form-control">
          ...
          <select class="col-9 input-form" id="form-control-3" name="category">
            <?php foreach ($categories as $category) : ?>
              <option class="form-label" <?php if(isset($product['category_id']) && $product['category_id'] && $product['category_id']==$category['id']) echo 'selected';?> value="<?= $category['id'] ?? '' ?>"><?= $category['name'] ?? '' ?></option>
            <?php endforeach ?>
          </select>
        </div>
